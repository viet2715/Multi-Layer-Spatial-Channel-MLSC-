...
if args.dataset == 'CWRU':
    ...
    acc = cal_accuracy_fewshot_1shot(test_dataloader_CWRU, net, device = args.device)
...
if args.dataset == 'PDB':
    ...
    net = Model().to(args.device)
    net = torch.load(args.best_weight)
    acc = cal_accuracy_fewshot_1shot(test_dataloader_PDB, net, device = args.device)
    print('Accuracy on PDB test data:', acc)
